class Api::V1::SquadsController < Api::V1::BaseController
    ...
    def destroy
      @squad.destroy
      respond_to do |format|
        format.html { redirect_to api_v1_squads_url, notice: 'Squad was successfully destroyed.' }
    ...
    def reset_db
      if current_user.id == '1' || current_user.id == '2'
        ...
        redirect_to api_v1_squads_path, notice: 'Database has been reset.'
      elsif
        Rails.logger.debug("!!! Attempted to reset_db, not user_id 1")
        redirect_to api_v1_squads_path, notice: 'You must be user_id 1 to do that action.'
      end
    end
